...
mysql_cmd="mysql -u${DP_APP_DB_USER} -p${DP_APP_DB_PASS} -S ${DP_APP_DB_SOCKET} --skip-column-names -AB"
mysql_users=`$mysql_cmd -e "$sql"` || exitmsg 1 "$SCRIPT_NAME: Error on mysql client: $sql"
...
for line in $mysql_users; do
	...
	# create user
	msg "$SCRIPT_NAME: ${db_user}@${db_host} -> CREATE USER"
	echo "CREATE USER '${db_user}'@'${db_host}';" > "$user_filename"

	# update password
	msg "$SCRIPT_NAME: ${db_user}@${db_host} -> UPDATE PASSWORD"
	echo "UPDATE mysql.user SET password='${db_pass}' WHERE user='${db_user}' and host='${db_host}';" >> "$user_filename"

	# grants
	msg "$SCRIPT_NAME: ${db_user}@${db_host} -> SHOW GRANTS"
	( $mysql_cmd -e "SHOW GRANTS FOR '${db_user}'@'${db_host}';"; echo $? ) | sed 's/$/;/g' >> "$user_filename"
done
